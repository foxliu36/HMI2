//------------------------------------------------------------------------------
// <auto-generated>
//    這個程式碼是由範本產生。
//
//    對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//    如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace HMI.Entity
{
    using System;
    using System.Collections.Generic;
    
    public partial class TB_WKF_SITE_PREADDITIONAL
    {
        public string FLOW_ID { get; set; }
        public string TASK_ID { get; set; }
        public string PARENT_SOURCE_SITE_ID { get; set; }
        public int SITE_SEQ { get; set; }
        public string SITE_ID { get; set; }
        public string SUBFLOW_SITE_ID { get; set; }
        public string PREV_SITE_ID { get; set; }
        public string NEXT_SITE_ID { get; set; }
        public int TIMEOUT_COUNT { get; set; }
        public int SIGN_TYPE { get; set; }
        public string GENERAL_SIGNERS { get; set; }
        public string ALERT_SIGNERS { get; set; }
        public bool IS_MODIFIABLE { get; set; }
        public string SIGNED_ALERT { get; set; }
        public bool IS_SCRIPT { get; set; }
        public int SITE_TYPE { get; set; }
        public string CREATE_FROM { get; set; }
        public Nullable<System.DateTime> CREATE_DATE { get; set; }
        public string CREATE_USER { get; set; }
        public string MODIFY_FROM { get; set; }
        public Nullable<System.DateTime> MODIFY_DATE { get; set; }
        public string MODIFY_USER { get; set; }
        public string SITE_CODE { get; set; }
    }
}
